...
public class ImageCache {
  ...
  private void init(float memCacheSizePercent) {
    ...
    // Set up memory cache
    if (com.raffaele.squarecash4glass.BuildConfig.DEBUG) {
  ...
  /**
   * Calculates the memory cache size based on a percentage of the max available
   * VM memory. Eg. setting percent to 0.2 would set the memory cache to one
   ...
   * bytes as this will eventually be passed to construct a LruCache which takes
   * an int in its constructor.
   *
   * This value should be chosen carefully based on a number of factors Refer to
   * the corresponding Android Training class for more discussion:
