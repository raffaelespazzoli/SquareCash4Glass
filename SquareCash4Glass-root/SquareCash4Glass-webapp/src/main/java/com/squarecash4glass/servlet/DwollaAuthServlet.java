...
public class DwollaAuthServlet extends HttpServlet {
  private static final Logger LOG = Logger.getLogger(DwollaAuthServlet.class.getSimpleName());

  @Override
  protected void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
    LOG.info("in dwolla oauth flow");
    manageDwollaOauth(req, res);
   ...
   */
  private void manageDwollaOauth(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
    ...
    if (req.getParameter("code") != null) {
      ...
      AuthorizationCodeFlow flow = DwollaAuthUtil.newAuthorizationCodeFlow();
      
      try {
      
      
      tokenResponse = flow.newTokenRequest(req.getParameter("code")).setRedirectUri(WebUtil.buildUrl(req, "/oauth2callbackdwolla")).execute();
      ...
      flow.createAndStoreCredential(tokenResponse, userid + "dwolla");
    ...
    AuthorizationCodeFlow flow = DwollaAuthUtil.newAuthorizationCodeFlow();
    GenericUrl url = flow.newAuthorizationUrl().setRedirectUri(WebUtil.buildUrl(req, "/oauth2callbackdwolla"));
