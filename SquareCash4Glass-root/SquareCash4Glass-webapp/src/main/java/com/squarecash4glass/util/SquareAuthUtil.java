...
public class SquareAuthUtil {
  private static final Logger LOG = Logger.getLogger(SquareAuthUtil.class.getSimpleName());
  ...
  public static AuthorizationCodeFlow newAuthorizationCodeFlow() throws IOException {
    ...
    return new AuthorizationCodeFlow.Builder(BearerToken.authorizationHeaderAccessMethod(), httpTransport, jacksonFactory, new GenericUrl(prodURL + tokenEndpointURI),
        new ClientParametersAuthentication(applicationId, secret), applicationId, new GenericUrl(prodURL + codeEndpointURI).toString()).setScopes(scopes)
